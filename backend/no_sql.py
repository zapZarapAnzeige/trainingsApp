...
async def get_chat_overview_of_user(user_name: str):
    all_chats = await chats.find({"participants": user_name}).to_list(length=None)
    all_chats = [
        {"partnerName": chat, "id": participants.get("_id")}
        for participants in all_chats
        for chat in participants.get("participants")
        if chat != user_name
    ]

    print([a.get("partnerName") for a in all_chats])
    pass
